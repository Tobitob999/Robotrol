...
# =========================
root = tk.Tk()
root.title("RoboControl FluidNC v5.1")
...
class ExecuteApp(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        fixed_wrap.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)

        info = ttk.Label(fixed_wrap, text="Fix current TCP orientation and move in XYZ while keeping it.")
        info.pack(anchor="w", padx=6, pady=(4, 6))

        row_fix = ttk.Frame(fixed_wrap)
        row_fix.pack(fill=tk.X, padx=6, pady=4)
        ...
        row_vals.pack(fill=tk.X, padx=6, pady=4)
        ttk.Label(row_vals, text="Roll").pack(side=tk.LEFT, padx=(0, 2))
        ttk.Entry(row_vals, textvariable=self.fixed_tcp_roll, width=8, justify="right").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(row_vals, text="Pitch").pack(side=tk.LEFT, padx=(0, 2))
        ttk.Entry(row_vals, textvariable=self.fixed_tcp_pitch, width=8, justify="right").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(row_vals, text="Yaw").pack(side=tk.LEFT, padx=(0, 2))
        ttk.Entry(row_vals, textvariable=self.fixed_tcp_yaw, width=8, justify="right").pack(side=tk.LEFT, padx=(0, 8))

        row_ctrl = ttk.Frame(fixed_wrap)
        row_ctrl.pack(fill=tk.X, padx=6, pady=(8, 4))
        ttk.Label(row_ctrl, text="Step [mm]").pack(side=tk.LEFT, padx=(0, 4))
        ttk.Entry(row_ctrl, textvariable=self.fixed_tcp_step, width=8, justify="right").pack(side=tk.LEFT, padx=(0, 10))
        ttk.Label(row_ctrl, text="Feed [mm/min]").pack(side=tk.LEFT, padx=(0, 4))
        ttk.Entry(row_ctrl, textvariable=self.fixed_tcp_feed, width=10, justify="right").pack(side=tk.LEFT)

        jog = ttk.LabelFrame(fixed_wrap, text="Move XYZ (relative)")
        jog.pack(fill=tk.BOTH, padx=6, pady=(8, 6))
        ...
        def _offset_to_world(dx, dy, dz, tcp):
            ...
            dxw = R[0][0] * dx + R[0][1] * dy + R[0][2] * dz
            dyw = R[1][0] * dx + R[1][1] * dy + R[1][2] * dz
            dzw = R[2][0] * dx + R[2][1] * dy + R[2][2] * dz
            return dxw, dyw, dzw
        ...
        def _make_slider_row(label, var):
            row = ttk.Frame(jog)
            row.pack(fill=tk.X, pady=2)
            ttk.Label(row, text=label, width=6).pack(side=tk.LEFT)
            scale = ttk.Scale(row, from_=-200.0, to=200.0, variable=var, length=260)
        ...
        sliders.pack(fill=tk.BOTH, expand=True, pady=(6, 0))
        _make_slider_row("X [mm]", self.fixed_tcp_dx)
        _make_slider_row("Y [mm]", self.fixed_tcp_dy)
        _make_slider_row("Z [mm]", self.fixed_tcp_dz)
        ...
        planes = ttk.Frame(jog)
        planes.pack(pady=(4, 0), fill=tk.X)
