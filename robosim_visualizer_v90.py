...
# Weltkoordinaten sind in cm, UI-Schritte sind in mm
...
def dir_to_yaw_tilt(t_world: np.ndarray):
    """Berechne Yaw/Tilt (Grad) aus einem Welt-Richtungsvektor."""
...
def fk_points(geom: Geometry, pose: Pose):
    """
    Vollständig korrekte FK für Thor/6DOF:
      A = global Yaw (um Z)
      X = Schulter Pitch
      Y = Ellbogen Pitch
      Z = Wrist Pitch
    ...
    def rot_axis(v, axis, ang_cos, ang_sin):
        axis = axis / (np.linalg.norm(axis) + 1e-12)
        return v * ang_cos + np.cross(axis, v) * ang_sin + axis * np.dot(axis, v) * (1 - ang_cos)

    # rotierte Richtungsbasis für Tool-Roll
    dir_tool_local = math.cos(th_z) * axis + math.sin(th_z) * n1r
    dir_tool_local /= np.linalg.norm(dir_tool_local)

    # *jetzt* roll um C anwenden
    dir_tool_local = rot_axis(dir_tool_local, axis, cC, sC)
    ...
    pts = [Rz @ p for p in (P0, P1, P2, P3, P4)]
    dir_world = Rz @ dir_tool_local
    return pts, dir_world

def tcp_pose_5(geom, pose):
    pts, dir_tool = fk_points(geom, pose)
    X, Y, Z = pts[-1]
    tilt = math.degrees(math.acos(np.clip(np.dot(dir_tool/np.linalg.norm(dir_tool), [0,0,1]), -1.0, 1.0)))
    yaw = math.degrees(math.atan2(Y, X))
    return round(X,2), round(Y,2), round(Z,2), round(yaw,2), round(tilt,2)
...
class VisualizerApp(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # 👇 Höhe reduziert: war (16, 9) → jetzt (16, 6.5)
        fig = Figure(figsize=(16, 6.5), dpi=100)

        # Verhältnis oben/unten leicht zugunsten der unteren Reihe
        gs = fig.add_gridspec(
            2, 2,
        ...
        self.canvas.draw()
        # 👇 weniger vertical expand, damit Platz für GCode bleibt
        ...
        ttk.Label(bar, text="ΔTool (mm):").pack(side="left", padx=(10, 2))
    ...
    def on_udp(self, line):
        ...
        try:
            ...
            if data.get("type") == "abs":
                ...
                self.pose.C = float(data.get("C", 0))   # NEU
                if "S" in data:
                    self.pose.servo = float(data["S"])
        except Exception:
            pass  # kein Log mehr für UDP noise
    ...
    def fix_position(self):
        """Speichert die aktuelle Gelenkpose 1:1 vom Robo (G-Code-Position)."""
        self.fixed_pose = {
            "A": self.pose.A,
            "X": self.pose.X,
            "Y": self.pose.Y,
        ...
        self.log("🔒 Fix Position gespeichert (direkt aus G-Code)")
        self.log(line)

    def _workspace_probe(self, target_world: np.ndarray, t_world: np.ndarray, A_deg: float):
        """Repliziert die 2R-Reichweitenprüfung (nur fürs Log), um frühe Ursachen zu sehen."""
    ...
    def move_tool_axis(self, dist_mm: float):
        ...
        try:
            ...
            pts, dir_world = fk_points(self.geom, self.pose)
            ...
            # Orientierung fixieren: A/B aus Pose, Tilt aus aktueller Toolrichtung
            # (Tilt = Winkel gegen +Z; wir holen ihn robust aus FK)
            _, _, _, _, tilt_now = tcp_pose_5(self.geom, self.pose)
            ...
            self.log(f"ΔTool (mm)  : {dist_mm:+.3f}  →  (cm): {dist_world:+.3f}")
            ...
            if not sol:
                self.log("❌ IK: keine Lösung zurückgegeben.")
                self.log("==============================")
                return
            ...
            self.log(f"✅ Tool Move (Gelenke) → {gline}")
            self.log("==============================")
    ...
    def retreat_to_fix(self):
        """Fährt vom aktuellen TCP wieder auf gespeicherte Fix-Position zurück (G-Code-Ausgabe)."""
        if not hasattr(self, "fixed_pose"):
            self.log("⚠️ Keine Fix-Position gespeichert.")
            return
    ...
    def export_gcode(self):
        if not self.gcode_lines:
            self.log("⚠️ Kein G-code zum Speichern")
            return
        ...
        self.log(f"💾 G-code gespeichert: {path}")
    ...
    # ------------------ Draw ------------------
    def redraw(self):
        pts, dir_tool = fk_points(self.geom, self.pose)
        X, Y, Z, Yaw, Tilt = tcp_pose_5(self.geom, self.pose)
        self.tcp_var.set(f"TCP → X={X:.2f}  Y={Y:.2f}  Z={Z:.2f}  Yaw={Yaw:.2f}°  Tilt={Tilt:.2f}°")

        self.ax3d.clear()
        cols = ["#666", "#999", "#00aa55", "#00aa55", "#cc3333"]
        for i in range(len(pts) - 1):
            p1, p2 = pts[i], pts[i + 1]
            self.ax3d.plot([p1[0], p2[0]], [p1[1], p2[1]], [p1[2], p2[2]],
                           "o-", color=cols[i + 1], lw=2)
        ...
        # === Greifer (zwei parallele Linien, rollt mit A + B + C um Toolachse) ===
        ...
        try:
            ...
            # ---------------------------------------------------------
            # STABILER Referenzvektor (singularitätsfrei)
            # ---------------------------------------------------------
            up = np.array([0.0, 0.0, 1.0])
            v0 = np.cross(up, t)

            # Fallback wenn Tool ~ Z-Achse → zweiten Referenzvektor nehmen
