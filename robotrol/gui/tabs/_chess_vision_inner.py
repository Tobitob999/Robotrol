"""Inner ChessVisionUI widget — extracted from monolith chess tab."""
from __future__ import annotations
import os
import tkinter as tk
from tkinter import ttk

from robotrol.chess.vision_pipeline import detect_chess_state, load_config


class ChessVisionInner(ttk.Frame):
    """Full chess-vision UI: config reload, detect, FEN display, board render."""
    ...
    def __init__(self, master, camera_capture=None, logger=None):
        ...
        self._board_text: tk.Text | None = None
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        ttk.Button(header, text="Load Config", command=self._reload_config).grid(
            row=0, column=0, padx=(0, 6)
        )
        ttk.Button(header, text="Detect State", command=self._detect_state).grid(
        ...
        ttk.Entry(fen_frame, textvariable=self._fen_var, width=80).pack(
    ...
    def _detect_state(self):
        ...
        if image is None:
            ...
            return
        state, _detection = detect_chess_state(image, self._config, color_order="rgb")
        if state is None:
