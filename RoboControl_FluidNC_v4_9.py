...
# =========================
root = tk.Tk()
root.title("RoboControl FluidNC v4.9")
...
class ExecuteApp(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        def start_visualizer_window():
            ...
            if existing:
                print(f"ℹ️ Visualizer läuft bereits (PID={existing.pid}) – kein neuer Start.")
                return

            try:
                # Wenn EXE vorhanden (z. B. im dist-Ordner), diese bevorzugen
                if os.path.exists(VISUALIZER_EXE):
                    print(f"🚀 Starte Visualizer EXE: {VISUALIZER_EXE}")
                    subprocess.Popen([VISUALIZER_EXE], cwd=BASE_DIR)
                elif os.path.exists(VISUALIZER_SCRIPT):
                    print(f"🚀 Starte Visualizer PY: {VISUALIZER_SCRIPT}")
                    subprocess.Popen([sys.executable, VISUALIZER_SCRIPT], cwd=BASE_DIR)
                else:
                    print(f"❌ Kein Visualizer gefunden in {BASE_DIR}")
                    return

                time.sleep(0.5)
                print("✅ Visualizer erfolgreich gestartet.")
            except Exception as e:
                print(f"❌ Visualizer-Start fehlgeschlagen: {e}")
...
"""
    ...
    def add_current_pose_to_queue(self):
        """
        Nimmt die aktuellen Werte aller Achsen-Entries,
        clamped auf Achsen-/HW-Limits, baut einen G1-Satz
        und legt ihn in die Queue.
        Entspricht funktional dem Button "🚀 To Queue".
        """
        try:
            parts = []
            ...
            for ax in AXES:
                ...
                # Für "aktuelle Pose" bevorzugt die letzten Statuswerte verwenden.
                if ax in mpos:
                    v = mpos[ax]
                elif ax in wpos:
                    v = wpos[ax]

                # Fallback auf Entry-Wert
                if v is None:
                ...
                # Fallback auf Slider/Vars
                if v is None:
                    try:
                        v = float(self.axis_vars[ax].get())
                    except Exception:
                        v = 0.0

                # Clamp mit HW-Limits falls vorhanden, sonst mit Achsenlimits
                if ax in self.hw_limits:
            ...
            # Feedrate inkl. Manual-Speed-Faktor
            ...
            gline = "G1 " + " ".join(parts) + f" F{F:.0f}"
            self.enqueue(gline)
            self.log(f"Pose → Queue (Gamepad/Button): {gline}")
