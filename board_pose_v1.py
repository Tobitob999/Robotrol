# ============================================================================================
# ♟️ Board Pose Module v1.0
# 3D-Poseberechnung (solvePnP) + Statusanzeige unter dem Kamerabild
# ============================================================================================
...
class BoardPose:
    def __init__(self, master, camera, pattern_size=(7, 7), square_size_mm=30.0):
        self.master = master
        self.camera = camera
        ...
        # Falls keine Kamera-Instanz übergeben wurde → eigene erzeugen
        if camera is None:
            from camera_capturev_v1_1 import CameraCapture
            self.camera = CameraCapture(master, width=420, height=320)
        else:
            self.camera = camera
        ...
        self.indicators = {}
        for name, color in [("Erkannt", "green"), ("Verloren", "orange"), ("Nicht erkannt", "red")]:
    ...
    def _update_ui(self):
        ...
        if self.paused:
            # pausiert: keine neuen Frames holen
        ...
        rvec, tvec = None, None
        tracking_lost = False
        offset = -self.square_size / 1.0  # von v2.4 übernommen
...
#        cv2.putText(vis, txt, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
    ...
    def _camera_matrix(self, gray):
        """ Annahme: einfache Pinhole-Kamera mit f ≈ 800 px """
        h, w = gray.shape[:2]
        f = 0.8 * w
        cx, cy = w / 2, h / 2
    ...
    def _rotation_to_euler(self, R):
        """ Konvertiert Rotationsmatrix zu Euler (Yaw, Pitch, Roll) """
    ...
    def _update_indicators(self):
        ...
        if self.status == "ok":
            self.indicators["Erkannt"][0].itemconfig(self.indicators["Erkannt"][1], fill="limegreen")
        elif self.status == "lost":
            self.indicators["Verloren"][0].itemconfig(self.indicators["Verloren"][1], fill="gold")
        else:
            self.indicators["Nicht erkannt"][0].itemconfig(self.indicators["Nicht erkannt"][1], fill="red")
    ...
    def get_field_position(self, field_name: str):
        """
        Gibt die Weltkoordinaten (x,y,z) eines Schachfelds zurück.
        Feldbezeichnung wie 'E2' → Mittelpunktkoordinate.
        """
    ...
    def pause_feed(self):
        """Live-Bild pausieren (Frame bleibt stehen)."""
        self.paused = True
        self.pose_text.set("🟡 Live-Feed pausiert")

    def resume_feed(self):
        """Live-Bild fortsetzen, sicher nur ein Loop aktiv."""
        if not self.running:
            # falls vorher gestoppt: neu starten
            self.running = True

        if not self.paused:
            return  # schon aktiv, kein doppelstart
        self.paused = False
        self.pose_text.set("🟢 Live-Feed aktiv")

        # Nur EIN Loop aktiv: starte erst nach kleinem Delay,
        # damit kein doppelter .after()-Cycle aktiv bleibt
        self.master.after(100, self._resume_safe)

    def _resume_safe(self):
        """Hilfsfunktion – prüft, ob Loop wirklich fortgesetzt werden darf."""
        if self.running and not self.paused:
            self._update_ui()

    def stop(self):
        """Vision komplett stoppen."""
        self.running = False
        self.paused = True
        self.pose_text.set("🔴 Vision gestoppt")
...
if __name__ == "__main__":
    from camera_capturev_v1_1 import CameraCapture
    root = tk.Tk()
    root.title("♟️ Board Pose v1.0 – 3D Pose + Statusanzeige")
...
# Kamera–Roboter-Kalibrierung (Chessboard Alignment)
# ============================================================================================
class BoardDetectorCalibration(ttk.Frame):
    ...
    def _build_ui(self):
        ttk.Label(self, text="🧭 Kamera–Roboter Kalibrierung",
                  font=("Segoe UI", 11, "bold")).pack(pady=8)
        btnf = ttk.Frame(self)
        btnf.pack(pady=6)
        for f in ["A1", "H1", "A8", "H8"]:
            ttk.Button(btnf, text=f"Feld {f} erfassen",
                       command=lambda f=f: self.capture_point(f)).pack(side=tk.LEFT, padx=4)

        ttk.Button(self, text="📐 Kalibrieren (R|t berechnen)",
        ...
        self._log("Noch keine Punkte erfasst.\n")
    ...
    def capture_point(self, field):
        try:
            ...
            self._log(f"Erfasst {field}: CAM {np.round(p_cam,1)} → ROBOT {np.round(p_robot,1)}")
        except Exception as e:
            messagebox.showerror("Fehler beim Erfassen", str(e))
            self._log(f"[❌] Erfassen fehlgeschlagen: {e}")

    def compute_transform(self):
        if len(self.points_cam) < 3:
            messagebox.showwarning("Hinweis", "Mindestens 3 Punkte nötig!")
            return
        try:
            ...
            if H is None:
                raise ValueError("Affine2D konnte nicht berechnet werden.")
            R = np.eye(3)
            R[:2, :2] = H[:2, :2]
            t = np.zeros(3)
            t[:2] = H[:, 2]
            ...
            self._log(f"✅ Kalibrierung erfolgreich! RMSE={rmse:.2f} mm\nR=\n{np.round(R,3)}\nt={np.round(t,2)}")
        except Exception as e:
            messagebox.showerror("Kalibrierungsfehler", str(e))
            self._log(f"[❌] Kalibrierung fehlgeschlagen: {e}")

    def save_transform(self):
        if self.R is None or self.t is None:
            messagebox.showwarning("Hinweis", "Noch keine gültige Kalibrierung berechnet.")
            return
        ...
        if hasattr(self.execute_app, "save_cam_to_base"):
            ...
            messagebox.showinfo("Gespeichert", "✅ cam_to_base.json wurde angelegt.")
            self._log("💾 cam_to_base.json gespeichert.")
